    ...
    <div id="detail"><div class="inner">
      <div class="dtools">
        ...
        <div class="seg" role="group" aria-label="Projection">
          <button id="vGlobe" aria-pressed="true">Globe</button><button id="vGround" aria-pressed="false">Ground</button><button id="vFlat" aria-pressed="false">Flat</button>
        </div>
        <div class="zbtns">
...
function drawSky(){
  ...
  /* you, but only when the ground you are standing on is actually in shot */
  let onGround=false;
  for(const p of hp)if(p.x>-40&&p.x<w+40&&p.y>0&&p.y<h+60){onGround=true;break;}
  if(onGround){
  const fy=h-6,fx=w/2;
  sc.fillStyle="#171d27";
  sc.beginPath();sc.ellipse(fx,fy,26,5,0,0,6.2832);sc.fill();
  sc.strokeStyle="#2b3646";sc.lineWidth=2.2;sc.lineCap="round";
  sc.beginPath();sc.moveTo(fx,fy-8);sc.lineTo(fx,fy-20);
  sc.moveTo(fx,fy-11);sc.lineTo(fx-5,fy-16);sc.moveTo(fx,fy-11);sc.lineTo(fx+5,fy-16);
  sc.moveTo(fx,fy-8);sc.lineTo(fx-4,fy-1);sc.moveTo(fx,fy-8);sc.lineTo(fx+4,fy-1);
  sc.stroke();
  sc.fillStyle="#2b3646";sc.beginPath();sc.arc(fx,fy-23,3.2,0,6.2832);sc.fill();}
  /* readouts */
...
const TPLAY=42000;   /* one whole eclipse in about forty seconds, whatever its length */
...
function setTLab(){const e=sel;
  ...
  tDur.textContent=d>0?fd(d)+" "+(e.t===1?"annular":"total")
    +(tTimer?"  ×"+Math.round((win(e).b-win(e).a)*60/(TPLAY/1000)):""):"—";}
...
tPlay.onclick=()=>{if(tTimer){stopT();return;}
  ...
  const tick=now=>{if(!tTimer)return;
    let f=(now-t0)/TPLAY;
    if(f>=1){f=0;t0=now;}
    tFrac=f;tSlide.value=Math.round(f*1000);setTLab();repaint();
...
function setDetView(v){detView=v;
  ...
  vGround.setAttribute("aria-pressed",String(v==="ground"));
  ...
  gHint.textContent=v==="globe"?"drag to spin · tap to stand there"
    :v==="ground"?"drag to look · W A S D to walk":"flat overview";
  syncObs();repaint();}
vGlobe.onclick=()=>setDetView("globe");
vGround.onclick=()=>setDetView("ground");
...
["pointerup","pointercancel"].forEach(t=>globe.addEventListener(t,ev=>{
  if(t==="pointerup"&&!gMoved&&gPts.size===1&&!gPin){
    const k=camOf(globeBox.clientWidth||320,gSize||320);
    const ll=unpj(ev.offsetX,ev.offsetY,k);
    if(ll&&sel){const first=!obs;setObs(ll[0],ll[1]);
      if(first)setDetView("ground");}}
